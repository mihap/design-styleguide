...
if (fs.existsSync(path.join(tailwindRoot, 'src/tokens.json'))) {
  ...
  if (fs.existsSync(path.join(tailwindRoot, 'src/theme.css'))) {
    ...
    for (const name of Object.keys(tokens.spacing || {})) {
      if (!theme.includes(`--mp-space-${name.replace('.', '-')}:`)) errors.push(`theme.css missing spacing alias: --mp-space-${name.replace('.', '-')}`);
    }
    for (const name of Object.keys(tokens.buttonIconSizes || {})) {
      if (!theme.includes(`--mp-button-icon-${name}:`)) errors.push(`theme.css missing button icon token: --mp-button-icon-${name}`);
      if (!theme.includes(`@utility mp-icon-${name}`)) errors.push(`theme.css missing button icon utility: mp-icon-${name}`);
    }
  }
}
