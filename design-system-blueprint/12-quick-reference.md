# 12. Quick Reference

## Theme Variables

```
:root {
  color-scheme: [scheme — e.g. light / dark];
  --color-base-100: [token value];
  --color-base-200: [token value];
  --color-base-300: [token value];
  --color-base-content: [token value];
  --color-primary: [token value];
  --color-primary-content: [token value];
  --color-secondary: [token value];
  --color-secondary-content: [token value];
  --color-accent: [token value];
  --color-accent-content: [token value];
  --color-neutral: [token value];
  --color-neutral-content: [token value];
  --color-info: [token value];
  --color-info-content: [token value];
  --color-success: [token value];
  --color-success-content: [token value];
  --color-warning: [token value];
  --color-warning-content: [token value];
  --color-error: [token value];
  --color-error-content: [token value];
  --radius-selector: [rem];
  --radius-field: [rem];
  --radius-box: [rem];
  --size-selector: [rem];
  --size-field: [rem];
  --border: [rem];
  --depth: [number];
  --noise: [number];
}
```

---

## Color Quick Reference

| Name         | HEX     | OKLCH       | Tailwind                |
|--------------|---------|-------------|-------------------------|
| Primary      | [hex]   | [oklch]     | [utility classes]       |
| Secondary    | [hex]   | [oklch]     | [utility classes]       |
| Accent       | [hex]   | [oklch]     | [utility classes]       |
| Base 100     | [hex]   | [oklch]     | [utility classes]       |
| Base 200     | [hex]   | [oklch]     | [utility classes]       |
| Base 300     | [hex]   | [oklch]     | [utility classes]       |
| Base Content | [hex]   | [oklch]     | [utility classes]       |
| Info         | [hex]   | [oklch]     | [utility classes]       |
| Success      | [hex]   | [oklch]     | [utility classes]       |
| Warning      | [hex]   | [oklch]     | [utility classes]       |
| Error        | [hex]   | [oklch]     | [utility classes]       |

---

## Typography Quick Reference

| Element    | Size  | Weight   | Line Height  | Tailwind             |
|------------|-------|----------|--------------|----------------------|
| H1         | [rem]  | [weight] | [unitless]   | [utility classes]    |
| H2         | [rem]  | [weight] | [unitless]   | [utility classes]    |
| H3         | [rem]  | [weight] | [unitless]   | [utility classes]    |
| H4         | [rem]  | [weight] | [unitless]   | [utility classes]    |
| Body       | [rem]  | [weight] | [unitless]   | [utility classes]    |
| Body Large | [rem]  | [weight] | [unitless]   | [utility classes]    |
| Small      | [rem]  | [weight] | [unitless]   | [utility classes]    |
| Label      | [rem]  | [weight] | [unitless]   | [utility classes]    |
| Button     | [rem]  | [weight] | [unitless]   | [utility classes]    |

---

[Design System Name] Design System Guidelines · Version [X.Y]
For questions or updates, contact [owner / team].
