...
Creates a guide directory, copies blueprint chapters, copies validator templates, creates demo data/schema support, and writes an initial manifest. Generated demo/export artifacts are written later to `tmp/pif/demo` and `tmp/pif/export`, not into the guide. Use `--target` to write to an exact directory such as `docs/styleguide`; use `--out` to write under a parent directory with the generated slug. Use `--force` to refresh support files; add `--overwrite-chapters` only when replacing markdown chapters is intended.
...
Renders the demo into a disposable `tmp/pif/demo` workspace from the guide's `demo/demo-data.json`, with its own Tailwind package files and `dist/demo.css`. The directory is emptied before each run.
